// src/components/menu/CustomerMenuPage.tsx
"use client";
...
export default function CustomerMenuPage() {
  ...
  const tableId = (params.tableId ?? params.table) as string | undefined;
  const roomId = (params.roomId ?? params.room) as string | undefined;
  const targetId = tableId || roomId || "";
  const context: MenuContext = roomId ? "room" : "table";
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium">
              {context === "room" ? "Room" : "Table"} {targetId} â€¢ Dine-in
            </p>
          </div>
          ...
          <div key={item.id} className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex gap-4">
            <div className="flex-1 flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm font-bold text-gray-900 mt-1">â‚¹{item.price}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full rounded-t-3xl p-6 animate-in slide-in-from-bottom-10">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setSelectedItemForVariant(null)} className="p-2 bg-gray-100 rounded-full">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3 mb-6">
              {selectedItemForVariant.variants.map((variant: any) => (
                ...
                >
                  ...
                  <span className="font-bold text-gray-900">â‚¹{variant.price}</span>
                </button>
              ))}
            </div>
          </div>
                ...
                <div key={item.id} className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {item.variant !== "Standard" && (
                      <span className="text-xs text-orange-600 font-medium bg-orange-50 px-2 py-0.5 rounded">{item.variant}</span>
                    )}
                    <p className="font-bold text-gray-900 mt-1">â‚¹{item.price * item.quantity}</p>
                  </div>
        ...
        <div className="fixed bottom-4 left-4 right-4 z-30">
          ...
          >
            <div className="text-left">
              ...
              <p className="font-bold text-lg">â‚¹{Math.round(cartTotal * 1.05)}</p>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center max-w-xs w-full animate-in zoom-in-95 duration-300">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mb-6">
              <svg
                className="w-10 h-10 text-green-600 animate-in zoom-in spin-in-180 duration-700"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth="3"
            ...
            <p className="text-gray-500 text-center text-sm mb-6">
              Kitchen has received your order for {context === "room" ? "Room" : "Table"} {targetId}.
            </p>
